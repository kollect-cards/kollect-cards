...
import "./kollect-market-storage-v2.sol";
import "./interfaces/ikollect-1155-v1.sol";

contract KollectMarketLogic_V2 is 
    KollectMarketStorage_V2
{
    ...
    function _getPagedItems(SimpleMarket[] memory items, uint itemsPerPage, uint pageNum) internal pure returns (SimpleMarket[] memory _items, uint _cursor, uint _itemsPerPage, uint _total) {
        uint cursor = 0;
        if (itemsPerPage > 256) {
            itemsPerPage = 256;
        }       
        if ((itemsPerPage * pageNum) > items.length) {
            if (itemsPerPage > items.length) {
                itemsPerPage = items.length;
            }
            cursor = items.length - itemsPerPage;
        } else {
            cursor = (itemsPerPage * pageNum) - itemsPerPage;
        }
        SimpleMarket[] memory pagedItems = new SimpleMarket[](itemsPerPage);
        uint index = 0;
        for (uint i = cursor; i < (cursor + itemsPerPage); i++) {
            pagedItems[index] = items[i];
            index++;
        }
        return (pagedItems, cursor, itemsPerPage, items.length);
    }

    function _getPagedItemsProcessed(SimpleMarket[] memory items, uint itemsPerPage, uint itemsTotal) internal pure returns (SimpleMarket[] memory _items, uint _cursor, uint _itemsPerPage, uint _total) {
        SimpleMarket[] memory pagedItems = new SimpleMarket[](itemsPerPage);
        uint index = 0;
        ...
        return (pagedItems, 0, itemsPerPage, itemsTotal);
    }
    ...
    function fetchMarketItems() public view returns (SimpleMarket[] memory) {
        ...
        for (uint i = 0; i < simpleMarketIndex; i++) {
            if (!allSimpleMarkets[i].isSold) {
                uint currentId = i;
                SimpleMarket storage currentItem = allSimpleMarkets[currentId];
    ...
    function fetchMarketItemsWithPaging(uint itemsPerPage, uint pageNum, bool soldIncluded) public view 
        ...
        // return _getPagedItems(items, itemsPerPage, pageNum);
        // return _getPagedItemsProcessed(items, itemsPerPage, simpleMarketIndex);
